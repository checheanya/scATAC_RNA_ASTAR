...
```{r}
kidney <- readRDS("./temp_data/kidney_f_nf_annotated_withQC.RDS")
DefaultAssay(kidney) <- 'ATAC'
...
saveRDS(kidney, file='./temp_data/kidney_filtered_as_scanpy.RDS')
```
...
```{r}
saveRDS(kidney, file='./temp_data/kidney_annotated_withQC_filtered.RDS')
```

## Normalisation 

```{r}
kidney <- readRDS("./temp_data/kidney_annotated_withQC_filtered.RDS")
```
...
```{r}
kidney <- RunUMAP(object = kidney, reduction = 'lsi', dims=2:30)
kidney <- FindNeighbors(object = kidney, reduction = 'lsi', dims = 2:30)
kidney <- FindClusters(object = kidney, verbose = FALSE)
DimPlot(kidney, label = TRUE) + NoLegend()

saveRDS(kidney[['seurat_clusters']], file='./temp_data/clusters_lsi')
```
...
kidney <- RunUMAP(object = kidney, reduction = 'lsi', dims=2:30)
kidney <- FindNeighbors(object = kidney, dims = 2:30)
kidney <- FindClusters(object = kidney, verbose = FALSE)
DimPlot(kidney, label = TRUE) + NoLegend()

saveRDS(kidney[['seurat_clusters']], file='./temp_data/clusters_log_norm.RDS')
```
