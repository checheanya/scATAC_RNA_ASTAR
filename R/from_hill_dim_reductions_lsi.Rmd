...
plot_clustering_comparison = function(seurat_obj1, seurat_obj2, reduction, description1='', description2 = '', cluster_column1='RNA_snn_res.0.3', cluster_column2='RNA_snn_res.0.3') {
  ...
  p1 <- DimPlot(seurat_obj1, reduction = 'tsne') +
            ggtitle(description1)
  
  p2 <- DimPlot(seurat_obj2, reduction = 'tsne') +
  ...
  p3 <- DimPlot(seurat_obj1, reduction = reduction) +
          ggtitle(paste0(description1, ' (', description2, ' clusters)'))
  
  p4 <- DimPlot(seurat_obj2, reduction = reduction) +
           ggtitle(paste0(description2, ' (', description1, ' clusters)'))
  
  p1
}
```
## Run cisTopic and SnapATAC on the lymph node file


```{r}
pathTo10X <- './datasets/lymphoma_10x/'
fragments <- paste0(pathTo10X, 'lymph_node_lymphoma_14k_atac_fragments.tsv.gz')
regions <-  paste0(pathTo10X, 'lymph_node_lymphoma_14k_atac_peaks.bed')
metrics <- paste0(pathTo10X, 'lymph_node_lymphoma_14k_per_barcode_metrics.csv')
...
In the initial tutorial mouse brain 10x genomics dataset was used, I will use my lymph node dataset instead. 

```{r}
dir.create('data_downloads', showWarnings = FALSE)

# Cusanovich and Hill et al.
# wget http://krishna.gs.washington.edu/content/members/ajh24/mouse_atlas_data_release/matrices/atac_matrix.binary.qc_filtered.rds
# wget http://krishna.gs.washington.edu/content/members/ajh24/mouse_atlas_data_release/metadata/cell_metadata.tissue_freq_filtered.txt
...
# CisTopic outputs for 1st dataset
# wget https://krishna.gs.washington.edu/content/members/ajh24/2019_04_19_atac_dim_reduction_blogpost/data_release/WholeBrain_PFC.cistopic.rds

# *.snap files produced by SnapTools for use with SnapATAC - here only for the 1st dataset
# wget  https://krishna.gs.washington.edu/content/members/ajh24/2019_04_19_atac_dim_reduction_blogpost/data_release/atac_v1_adult_brain_fresh_5k.snap
...
```{r}
lymph.bmat = readRDS("datasets/lymphoma_10x/atac_matrix.binary.qc_filtered.rds")
